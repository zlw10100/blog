...
单纯使用双向循环链表，每次在缓存中搜索的时候需要O(n)的复杂度，可以引入一个字典来降低到O(1)
...
class LRUCachePro(object):
    ...
    def __init__(self, max_size=128):
        ...
        # 使用字典来存储每一个链表节点的索引，以便搜索缓存的时候以O(1)速度
        self.map = dict()

    def get(self, key):
        target_value = None
        target_node = self.map.get(key)
        if  target_node is not None:  # hit
            # 移动该节点到头部位置
            self.link.append_left(self.link.delete(target_node))
            target_value = target_node.value[1]
        return target_value
    ...
    def save(self, key, value):
        ...
        if pop_size > 0:
            ...
            for i in range(pop_size):
                pop_node = self.link.pop()
                self.map.pop(pop_node.value[0])
                self.cur_size -= 1
